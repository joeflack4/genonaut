...
class TestBaseRepositoryPagination:
    ...
    def test_get_paginated_efficient_count_with_window_function(self):
        ...
        # Arrange
        mock_items_with_count = [
            type('Item', (), {'item': MockModel(1), 'total_count': 100})(),
            type('Item', (), {'item': MockModel(2), 'total_count': 100})(),
            type('Item', (), {'item': MockModel(3), 'total_count': 100})()
        ]

        # Mock the query to return items with count
        mock_subquery = Mock()
        mock_subquery.subquery.return_value = mock_subquery
        self.mock_query.add_columns.return_value = mock_subquery

        mock_query_with_count = Mock()
        self.mock_db.query.side_effect = [self.mock_query, mock_query_with_count]

        mock_query_with_count.offset.return_value = mock_query_with_count
        mock_query_with_count.limit.return_value = mock_query_with_count
        mock_query_with_count.all.return_value = mock_items_with_count
    ...
    def test_get_paginated_cursor_based(self):
    ...
    @pytest.mark.skip(reason="Data scaling tests - Mock architecture and cursor navigation issues (see scratchpads/issues/by_priority/low/data-scaling-tests.md)")
    def test_get_paginated_response_includes_cursors(self):
        ...
        pagination_request = PaginationRequest(page=2, sort_field="created_at")
